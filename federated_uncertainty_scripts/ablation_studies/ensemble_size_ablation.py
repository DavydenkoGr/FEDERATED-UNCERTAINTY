...
# Ablation study: ensemble sizes to test
ensemble_sizes = [1, 2, 3, 5, 7, 10]

print(f"\n{'='*60}")
print(f"ENSEMBLE SIZE ABLATION STUDY")
...
# create spoiler versions of our models from pool (created once, reused for all ensemble sizes)
spoilers = []
for model in ensemble:
    spoiler = get_noisy_model(model, noise_type, device, noise_level=spoiler_noise)
    spoilers.append(spoiler)

# Ablation loop over ensemble sizes
for current_ensemble_size in ensemble_sizes:
    if current_ensemble_size > n_models:
        print(f"  Skipping ensemble_size={current_ensemble_size} (exceeds n_models={n_models})")
        continue
    
    print(f"\n{'='*60}")
    print(f"ENSEMBLE SIZE: {current_ensemble_size}")
    print(f"{'='*60}")
    
    # Create separate directory for this ensemble size
    run_dir_ensemble = run_dir / f"ensemble_size_{current_ensemble_size}"
    run_dir_ensemble.mkdir(parents=True, exist_ok=True)
    
    print("MODEL SELECTION STRATEGIES COMPARISON")
    print(f"The model pool (`ensemble`) consists of {n_models} models.")
    print(f"For each of the {n_clients} clients, an ensemble of {current_ensemble_size} models will be selected.")
    print(f"Lambda: {lambda_disagreement}, Spoiler Noise: {spoiler_noise}")
    print(f"Results will be saved to: {run_dir_ensemble}")
    ...
    for i in range(n_clients):
        ...
        # save_logits_and_labels(hybrid_ensemble, client_ind_test_loaders[i], client_ood_test_loaders[i], i + 1, "hybrid", device, run_dir_ensemble)

print(f"\n{'='*60}")
print(f"ABLATION STUDY COMPLETED")
print(f"Results saved in subdirectories: {[f'ensemble_size_{size}' for size in ensemble_sizes]}")
print(f"{'='*60}\n")