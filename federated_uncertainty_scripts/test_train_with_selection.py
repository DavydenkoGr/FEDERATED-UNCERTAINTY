...
n_models = 20
n_clients = 5
LAMBDA = 0.2 # для Uncertainty-Aware
fraction = 4
n_epochs = 3
lr = 1e-3
...
for i in range(n_clients):
    ...
    print(f"  Client {i+1} has data for classes {selected_classes} with {len(test_indices)} test samples.")
...
for i in range(n_clients):
    ...
    client_test_loader = client_test_loaders[i]
    ...
    accuracy_random = evaluate_selected_ensemble(selected_ensemble_random, client_test_loader, device, criterion)
    print(f"  -> Точность ансамбля: {accuracy_random:.4f}%")
    
    print("  --- Стратегия: Выбор по точности (Accuracy-only) ---")
    selected_ensemble_acc = select_accuracy_only_models(ensemble, ensemble_selection_size, client_test_loader, device)
    accuracy_acc = evaluate_selected_ensemble(selected_ensemble_acc, client_test_loader, device, criterion)
    print(f"  -> Точность ансамбля: {accuracy_acc:.4f}%")

    print(f"  --- Стратегия: Uncertainty-Aware (lambda={LAMBDA}) ---")
    selected_ensemble_unc = select_uncertainty_aware_models(ensemble, ensemble_selection_size, client_test_loader, ood_loader, LAMBDA, device, criterion)
    accuracy_unc = evaluate_selected_ensemble(selected_ensemble_unc, client_test_loader, device, criterion)
