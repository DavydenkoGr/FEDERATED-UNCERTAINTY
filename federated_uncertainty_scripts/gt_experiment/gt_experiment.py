...
def select_ground_truth_models(models, num_to_select, client_test_loader, device, criterion):
    """
    Exhaustive search over all ensemble combinations to find the optimal set.
    Uses client test data to evaluate the accuracy of each combination.
    """
    print(f"    [Ground Truth] Exhaustive search over all {num_to_select}-model combinations...")
    
    n_models = len(models)
    total_combinations = _combinations_count(n_models, num_to_select)
    print(f"    [Ground Truth] Total combinations to evaluate: {total_combinations}")
    
    if total_combinations > 100000:
        print(f"    [Ground Truth] WARNING: Large number of combinations ({total_combinations}). This may take a long time.")
    
    best_accuracy = -1.0
    best_indices = None
    
    # Get all logits and labels once
    all_logits, targets = get_logits_and_labels(models, client_test_loader, device)
    
    # Iterate over all combinations
    for idx, combination in enumerate(itertools.combinations(range(n_models), num_to_select)):
        if idx % 100 == 0 and idx > 0:
            print(f"    [Ground Truth] Progress: {idx}/{total_combinations} combinations evaluated...")
        
        # Compute ensemble logits for this combination
        ensemble_logits = all_logits[list(combination)].mean(dim=0)
        preds = ensemble_logits.argmax(dim=1)
        ...
        if accuracy > best_accuracy:
            ...
            best_indices = list(combination)
    
    print(f"    [Ground Truth] Best ensemble accuracy: {best_accuracy:.4f}")
    print(f"    [Ground Truth] Best ensemble indices: {best_indices}")
    
    return best_indices


def save_selected_models_indices(ensemble_indices, client_num, strategy, run_dir):
    """
    Save selected model indices for each strategy to a JSON file.
    """
    selected_models_path = run_dir / "selected_models" / f"client_{client_num}" / f"{strategy}.json"
    selected_models_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    print(f"  -> Saved selected models indices to: {selected_models_path}")
...
def select_and_evaluate_models(
    ...
    if strategy == "random":
        ...
        print("\n  --- Strategy: Ground Truth (Exhaustive Search) ---")
        ensemble_indices = select_ground_truth_models(
            spoilers, ensemble_size, client_ind_test_loader, device, criterion
        )
    else:
    ...
    save_selected_models_indices(ensemble_indices, client_num, strategy, run_dir)
...
for i in range(n_clients):
    ...
    selected_ensemble_gt, indices_gt = select_and_evaluate_models(
        "ground_truth",
        ensemble,
        spoilers,
        client_ind_train_loaders[i],
