...
def process_multidimensional_composition(
    ...
    try:
        ...
        for group_key, group_data in pred_data.items():
            ...
            if args.scaler_type == "global":
                metric_wise_scaled_maximums = scaler.local_max_ / scaler.global_max_
            else:
                metric_wise_scaled_maximums = 1
            maximal_elements_matrix = (
                np.vstack(maximal_elements)
                * args.grid_size
                * metric_wise_scaled_maximums
            )  # we used 2
            uncertainty_matrix_calib = np.vstack(
                [uncertainty_matrix_calib, maximal_elements_matrix]
            )

            train_loader = torch.utils.data.DataLoader(
                torch.tensor(
                    uncertainty_matrix_calib, dtype=torch.float32, device="cpu"
                ),
                batch_size=128,
                shuffle=False,
            )
...
def main():
    ...
    parser.add_argument(
        "--grid_size", type=int, default=2, help="EntropicOT grid size (default: 2)"
    )
    parser.add_argument(
        "--scaler_type",
        type=str,
        default="feature_wise",
